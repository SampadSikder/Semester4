...
key_t k1 = 5491, k2 = 5812, k3 = 4327, k4 = 3213; //identify unique memory spaces
bool* SHM1;
int* SHM2;
int* SHM3;
...
int main()
{
	...
	SHM3 = (int*)shmat(shmid3, NULL, 0);//shared memory attach null pathaile je pointer dise setay attached
	int ix = 0;
	...
	*state = 1;
	...
	int i = 0; // Consumer turn
	int j = 1; // Producer turn

	if (fork() == 0) // Producer code
	{
		SHM1 = (bool*)shmat(shmid1, NULL, 0);//flag
		SHM2 = (int*)shmat(shmid2, NULL, 0);//turn
		SHM3 = (int*)shmat(shmid3, NULL, 0);//buffer
		if (SHM1 == (bool*)-1 || SHM2 == (int*)-1 || SHM3 == (int*)-1) {//error hoile -1 return kore
	...
	if(fork()==0)// Consumer code //keno starve hocche jokhon eta parent? 3ta process
	{
		SHM1 = (bool*)shmat(shmid1, NULL, 0);//flag
		SHM2 = (int*)shmat(shmid2, NULL, 0);//turn
		SHM3 = (int*)shmat(shmid3, NULL, 0);//buffer
